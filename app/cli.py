...
def run_start(*, no_open: bool = False, foreground: bool = False, regenerate_secrets: bool = False) -> int:
    ...
    if regenerate_secrets or not LOCAL_ENV_FILE.exists():
        previous = _read_environment_file(LOCAL_ENV_FILE)
        generated = _local_environment_values()
        if previous.get("POSTGRES_PASSWORD"):
            generated["POSTGRES_PASSWORD"] = previous["POSTGRES_PASSWORD"]
        _write_environment_file(LOCAL_ENV_FILE, generated)
        print(f"Created private local configuration: {LOCAL_ENV_FILE}")
    values = _read_environment_file(LOCAL_ENV_FILE)
    required = {
        "AEGIS_SETUP_TOKEN",
        "POSTGRES_PASSWORD",
    ...
    running = subprocess.run(
        _docker_compose_command(LOCAL_ENV_FILE, "ps", "--status", "running", "--quiet"),
        capture_output=True,
        text=True,
        check=False,
    )
    if not running.stdout.strip():
        occupied = [str(port) for port in (80, 443) if not _port_is_available(port)]
        if occupied:
            print(
    ...
    command = _docker_compose_command(LOCAL_ENV_FILE, "up", "--build")
    if not foreground:
        command.append("-d")
    ...
    if not _wait_for_dashboard(base_url):
        print(
...
def run_stop() -> int:
    if not _require_local_stack():
        return EXIT_OPERATIONAL_ERROR
    return subprocess.run(
        _docker_compose_command(LOCAL_ENV_FILE, "down"),
        cwd=PROJECT_ROOT,
        check=False,
    ).returncode
...
def run_stack_logs(*, follow: bool = False) -> int:
    if not _require_local_stack():
        return EXIT_OPERATIONAL_ERROR
    command = _docker_compose_command(LOCAL_ENV_FILE, "logs", "--tail", "200")
    if follow:
...
def run_upgrade(*, no_start: bool = False) -> int:
    ...
    if git_directory.exists():
        ...
        if dirty.stdout.strip():
            ...
            return EXIT_OPERATIONAL_ERROR
        result = subprocess.run(
            ["git", "pull", "--ff-only"], cwd=PROJECT_ROOT, check=False
        )
    else:
        result = subprocess.run(
...
def run_backup(output: str | None = None) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="aegis-backup-") as temporary:
        ...
        copy = subprocess.run(
            _docker_compose_command(
                LOCAL_ENV_FILE, "cp", "dashboard:/data/scans", str(root / "scans")
            ),
            cwd=PROJECT_ROOT,
            check=False,
        )
        ...
        (root / "manifest.json").write_text(
            json.dumps({"created_at": datetime.now(timezone.utc).isoformat(), "version": get_package_version()}, indent=2)
        )
...
def run_restore(archive_path: str, *, confirmed: bool = False) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="aegis-restore-") as temporary:
        ...
        subprocess.run(
            _docker_compose_command(LOCAL_ENV_FILE, "stop", "dashboard", "worker"),
            cwd=PROJECT_ROOT,
            check=False,
        )
        restore = subprocess.run(
            _docker_compose_command(
                LOCAL_ENV_FILE, "exec", "-T", "postgres", "psql", "-U", "aegis", "-d", "aegis"
            ),
            cwd=PROJECT_ROOT,
        ...
        if scans.exists():
            subprocess.run(
                _docker_compose_command(LOCAL_ENV_FILE, "cp", str(scans), "dashboard:/data/"),
                cwd=PROJECT_ROOT,
                check=False,
            )
        return subprocess.run(
            _docker_compose_command(LOCAL_ENV_FILE, "up", "-d", "dashboard", "worker"),
            cwd=PROJECT_ROOT,
            check=False,
        ).returncode
